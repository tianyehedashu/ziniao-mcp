...
| JSON | `{ success, data, error }`；可选 **`_boundary`** | **`--json`** 对齐信封；**`--json-legacy`** 原始 daemon 字典；**`--content-boundaries`** 在 JSON 顶层加 **`_boundary`**（见 [cli-json.md](cli-json.md)、[cli-llm.md](cli-llm.md)） |
| 人类可读输出 | Rich 表格 / `AGENT_BROWSER_PAGE_CONTENT` 边界行等 | Rich 表格；页面类长文本可用 **`ZINIAO_PAGE_CONTENT`** 边界行（**`--content-boundaries`**）；**`--max-output`** 截断（stdout **默认 2000** 字符若未设置；**`-o` 文件全量**） |
