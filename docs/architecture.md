...
ziniao-mcp 采用三层架构，将 AI Agent 的意图转化为对紫鸟店铺浏览器的实际操作：
...
│                                                           │
│  ┌─────────┐  ┌──────────────┐  ┌──────────────────────┐ │
│  │ server  │→ │ SessionManager│→ │ tools/* (31 tools)   │ │
│  │ 配置解析 │  │ 会话 + CDP   │  │ store/input/nav/...  │ │
│  └─────────┘  └──────┬───────┘  └──────────────────────┘ │
└──────────────────────┼───────────────────────────────────┘
                       │
          ┌────────────┴────────────┐
          ...
          ▼                         ▼
┌──────────────────┐   ┌────────────────────────┐
│ ziniao_webdriver  │   │ Playwright CDP         │
│ HTTP → 紫鸟客户端 │   │ connect_over_cdp()     │
│ (启动/关闭/列表)  │   │ (页面操作/截图/JS执行)  │
└────────┬─────────┘   └────────────┬───────────┘
         │                          │
         ▼                          ▼
┌──────────────────┐   ┌────────────────────────┐
│ 紫鸟客户端进程     │   │ 店铺浏览器实例           │
│ (ziniao.exe)      │──→│ (Chromium + CDP 端口)   │
└──────────────────┘   └────────────────────────┘
```
...
`SessionManager` 是核心协调层，管理：

1. **客户端生命周期**：自动检测客户端是否运行，按需启动
2. **CDP 连接**：通过 Playwright 的 `connect_over_cdp()` 连接到店铺浏览器
3. **多店铺会话**：维护 `store_id → StoreSession` 映射，支持多店铺并行
4. **页面事件追踪**：自动监听 console、network、dialog 事件
5. **状态持久化**：将已打开店铺的 CDP 信息写入 `~/.ziniao/sessions.json`，支持跨进程恢复
...
| 模块 | 依赖 | 说明 |
|------|------|------|
| `store.py` | `SessionManager` 全部方法 | 店铺生命周期管理 |
| `input.py` | `session.get_active_page()` | Playwright 页面交互 |
| `navigation.py` | `session.get_active_page()` + `get_active_session()` | 页面/标签页导航 |
| `emulation.py` | `session.get_active_page()` + `session._playwright` | 设备模拟 |
| `network.py` | `session.get_active_session()` | 读取已捕获的网络数据 |
| `debug.py` | `session.get_active_page()` + `get_active_session()` | JS 执行、截图、控制台 |
...
Playwright.connect_over_cdp("http://127.0.0.1:9222")
    │ 建立 CDP 连接
    ▼
创建 StoreSession（browser, context, pages）
...
httpx GET http://127.0.0.1:{cdp_port}/json/version
    │ 检查 CDP 端口是否存活
    ▼
├── 存活 → Playwright.connect_over_cdp() → 创建 StoreSession
