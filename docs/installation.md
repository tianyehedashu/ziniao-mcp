...
ziniao 提供 MCP 工具（紫鸟店铺 + Chrome 浏览器）和 Cursor Plugin 两种使用形态，按需选择。
...
### 方式一：通过 uv 安装 ziniao（推荐，最简单）

无需 Python、无需 git，只需安装 [uv](https://docs.astral.sh/uv/)，再安装 ziniao 即可。MCP 配置中统一使用 `ziniao` 命令。

> 此方式只包含 MCP Server（全部工具），不含 Rules / Skills / Agents 等插件组件。如需完整 AI 增强体验请用方式二。
...
安装后若终端无法识别 `ziniao`，请将 `uv tool dir` 输出的目录加入用户 PATH。
...
**与 agent-browser 全量对照**（命令、参数、batch、snapshot 语义等）：[cli-agent-browser-parity.md](cli-agent-browser-parity.md)。

**大模型 / Agent**：使用 **`ziniao --json`**（或环境变量 **`ZINIAO_JSON=1`**）；需要与页面正文隔离时加 **`--content-boundaries`**（对标 agent-browser）；长输出用 **`--max-output N`**（未设置时 stdout 默认截断 2000 字符；**`0`** 表示不截断；**`-o` 写文件全量**）。详见 [cli-llm.md](cli-llm.md)。
...
2. 手动运行测试：`ziniao --help`（方式一）或 `uv run ziniao --help`（方式二，在插件目录下）
3. 检查环境变量是否正确配置
4. 方式一用户若怀疑是旧版本缓存导致：`uv tool install --upgrade ziniao` 后再试
