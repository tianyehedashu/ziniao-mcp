...
**浏览器生命周期**：紫鸟客户端以 WebDriver 模式启动，在本地监听 HTTP 端口（默认 16851）。MCP 服务器通过该端口控制客户端，再通过 nodriver 连接到各店铺浏览器实例的 CDP 端口。客户端会在首次需要时自动启动。
...
- **CSS 选择器**：`#submit-btn`、`.product-title`、`input[name="price"]`、`input[type=text]`、`.login-button`

优先使用具有唯一性的选择器（ID > 属性 > 类名）。

## CLI 等效命令

上述所有操作也可通过 `ziniao` CLI 完成：

| MCP 工具 | CLI 命令 |
|----------|----------|
| `navigate_page(url)` | `ziniao navigate <url>` |
| `take_snapshot` | `ziniao snapshot` |
| `take_screenshot` | `ziniao screenshot [file]` |
| `click(sel)` | `ziniao click <sel>` |
| `fill(sel, val)` | `ziniao fill <sel> <val>` |
| `type_text(text)` | `ziniao type <text>` |
| `wait_for(sel)` | `ziniao wait <sel>` |
| `evaluate_script(js)` | `ziniao eval "<js>"` |
| `tab(list)` | `ziniao tab list` |
| `console` | `ziniao info console` |
| `network` | `ziniao info network` |

CLI 支持 `--json` 输出和 `--store`/`--session` 多会话定向，适合脚本编排和批量操作。
