...
默认执行环境是 `browser_fetch`：在目标 session 的活动 tab 中读取 Cookie/storage/eval。`direct_http` / `auto` 使用 CookieVault AuthSnapshot 在 Python 侧静态解析 Cookie/storage；其中 `eval` 无浏览器上下文，直连模式会跳过。Agent 编排时必须用 `--store` / `--session` 固定目标会话，避免读到另一个 active tab 的登录态。
...
| `auth_snapshot_path` | `direct_http` / `auto` 所需的 AuthSnapshot JSON |
...
_page_fetch（daemon）→ _normalize_header_inject(args)  ← CLI 与 MCP 唯一归一化点
       ↓
transport 选择（browser_fetch / direct_http / auto）
       ↓
...
- **`api_transport.direct_http_fetch`**：从 AuthSnapshot 静态解析 `cookie` / `localStorage` / `sessionStorage` 注入；`eval` 不执行。该模式不复用浏览器网络环境，强风控站点默认不要使用。
...
在 `direct_http` 中，`eval` 规则会被跳过；如果鉴权依赖 DOM / window 运行时状态，必须使用 `browser_fetch` 或 `mode: "js"`。
...
| `direct_http` 401 / 风控 | 回到 `browser_fetch`；直连不复用紫鸟/Chrome 网络、TLS、指纹环境。 |
