...
- Recording: recorder(...); replay saves session_id/backend_type; if daemon has no session, replay auto connect_store/connect_chrome unless auto_session=false. New tab by default (reuse_tab=true for current tab).
...
- **Replay**: recorder(action='replay', name='...') or recorder(action='replay', actions_json='[...]'). By default opens a **new tab** (start URL from recording, or about:blank). If the MCP daemon has **no active browser session**, replay tries **auto_session** (default true) using session_id/backend_type/cdp_port from the saved recording. Set auto_session=false to disable. Use reuse_tab=true for current tab. Use speed to adjust pace.
