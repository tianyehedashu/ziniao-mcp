...
3. **RPA Flows（`ziniao flow …`）** — `kind: rpa_flow` 声明式浏览器流程，支持控制流、重试、文件 IO、HTTP/MCP 外呼、失败工件与断点恢复。
4. **Skills（`ziniao skill …`）** — 把 `ziniao-cli` 等业务技能一键安装到 Cursor / Claude Code / Trae / OpenClaw 等 Agent 的全局目录。
...
ziniao skill install rpa-flows                # 声明式 RPA flow 设计与调试
...
| `mode: ui`    | 历史兼容       | 旧版站点 UI 预设入口；新浏览器流程请优先写 `kind: rpa_flow`，底层复用同一套 UI steps 与 `flow_run` 执行路径 |
...
| `rpa-flows`                              | 设计、编写、调试 `kind: rpa_flow` 声明式 RPA 流程              |
...
ziniao rec stop [--name <名>] [--emit nodriver,playwright,preset] [--redact-secrets]
ziniao rec replay <名> [--speed 1.0] / list / delete <名> / status

ziniao flow validate ./flow.json
ziniao flow dry-run ./flow.json --plan
ziniao flow run ./flow.json --var key=value [--vars-from data.csv] [--run-dir ./.runs/demo]
ziniao flow list / flow show <run_id> / flow diagnose <run_id> --emit nodriver
...
遵循「探索 → 验证 → 固化」：`snapshot --interactive` 选择器 → `click` / `fill` → `wait` 或 `snapshot` 验证 → 写成 `kind: rpa_flow` JSON，或交给 `store-rpa-scripting` 技能生成不依赖 daemon 的独立 Python 脚本（`ziniao_webdriver` + `nodriver`）。

声明式 RPA Flow 适合交付给用户或 Agent 重复运行：

```bash
ziniao flow dry-run examples/rpa/minimal-rpa-flow.json --plan
ziniao flow run examples/rpa/flow-demo-login-and-extract.rpa-flow.json --var username=demo
```

失败会在 `~/.ziniao/runs/<run_id>/` 写出 `state.json` / `report.json` / 截图 / HTML；用 `ziniao flow diagnose <run_id> --emit nodriver` 生成复现脚本，用 `--replay <run_id> --resume-from step_id` 从指定步骤恢复。完整说明见 [docs/rpa-flows.md](docs/rpa-flows.md)。

录制：`ziniao rec start` → 在浏览器里操作 → `ziniao rec stop --name my-flow --emit nodriver,preset` 产出 `.json`（回放）+ `.py`（独立脚本）+ `.rpa-flow.draft.json`（RPA 草稿），`ziniao rec replay my-flow` 复现。草稿需要人工补变量、断言、循环和 `output_contract` 后再投入生产。
...
| [site-ui-flows.md](docs/site-ui-flows.md)                       | 历史 `mode: ui` 站点预设说明；新流程优先看 `rpa-flows.md` |
| [rpa-flows.md](docs/rpa-flows.md)                               | 统一 Flow：`kind: rpa_flow`、`ziniao flow` CLI、策略、调试与录制草稿 |
